...
export function QuickFilter() {
    ...
    return (
        <div className="container mx-auto px-4 -mt-16 relative z-20 w-full flex flex-col items-center">
            <FadeIn direction="up" delay={0.4} className="w-full max-w-4xl">
                <Card className="w-full shadow-2xl bg-white/95 backdrop-blur-xl border border-white/20 overflow-visible">
                    {/* Tab Switcher */}
                    <div className="flex border-b border-zinc-100">
                        <button
                            onClick={() => setActiveTab("book")}
                            className={cn(
                                "flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition-all hover:bg-zinc-50 relative",
                                activeTab === "book" ? "text-primary bg-blue-50/50" : "text-muted-foreground"
                            )}
                        >
                            <Trophy className="h-4 w-4 relative z-10" />
                            <span className="relative z-10">Book Court</span>
                            {activeTab === "book" && (
                                <motion.div
                                    layoutId="activeTab"
                                    className="absolute inset-0 border-b-2 border-primary bg-blue-50/50"
                                />
                            )}
                        </button>
                        <button
                            ...
                            className={cn(
                                "flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition-all hover:bg-zinc-50 relative",
                                activeTab === "match" ? "text-primary bg-blue-50/50" : "text-muted-foreground"
                            )}
                        >
